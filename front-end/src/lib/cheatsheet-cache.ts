// A simple cache for storing cheatsheet data to avoid redundant API calls

interface CheatsheetCacheItem {
    data: string
    timestamp: number
    documentIds: string[]
  }
  ...
  class CheatsheetCache {
    private cache: Map<string, CheatsheetCacheItem> = new Map()
    ...
    // Store cheatsheet data in the cache
    set(documentIds: string[], data: string): void {
      ...
      console.log(`Cached cheatsheet data for documents: ${documentIds.join(", ")}`)
    }
  
    /**
     * Determines if the cheatsheet needs to be refreshed based on selected files
    ...
    // Get cheatsheet data from the cache if it exists and is not expired
    ...
    clear(): void {
      this.cache.clear()
      console.log("Cheatsheet cache cleared")
    }
  }
  
  // Create a singleton instance
  const cheatsheetCache = new CheatsheetCache()
  export default cheatsheetCache
