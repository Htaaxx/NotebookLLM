...
@router.post("/")
async def embed_pdf(request: EmbedPDFRequest = Depends(), file: UploadFile = File(...)):
    """
...
async def delete_embed(request: DeleteEmbedRequest = None):
    ...
    try:
        ...
        if request.document_id is not None:
            ...
            return {
                "message": f"Successfully deleted document with ID: {request.document_id}"
            }
        ...
        if filter_dict:
            return {
                "message": f"Successfully deleted {deleted_count} embeddings matching the filter."
            }
        else:
            return {"message": "Successfully deleted all embeddings."}
    except Exception as e:
